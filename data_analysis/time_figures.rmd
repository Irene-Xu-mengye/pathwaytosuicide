...
plotbin <- function(data, x.var,  v.title = "", legend.pos = "none", y.max = 10, t.margin = 0, r.margin = 0) {
  ...
  ggplot(data, aes(x = x.var,  fill = group)) +
    ...
    scale_fill_manual(name = "Type of the Most Recent Attempt                ",
                      labels = c("Aborted/Interrupted", "Actual"),
...
p1 <- plotbin(time, 'think_f', "Started to Consider Attempting", "none", 10, 0.5, 0)
p2 <- plotbin(time, 'mull_f', "Started to Mull", "none", 14, 0.5, 0)
p3 <- plotbin(time, 'dec_f', "Decided to Attempt", "none", 10, 0.5, 0.5)

p4 <- plotbin(time, 'fm_f', "Started to Consider Method", "none")
p5 <- plotbin(time, 'sm_f', "Decided on Method", "right")
p6 <- plotbin(time, 'fp_f', "Started to Consider Place")
p7 <- plotbin(time, 'sp_f', "Decided on Place")
